...
{
  const char* input = "version: 123, pi: 3.14";
  IutfLexer* lexer = iutf_lexer_new (input);
  ...
  token = iutf_lexer_next (lexer);
  g_assert_cmpint (token.type, ==, IUTF_TOK_FLOAT);
...
int main(int argc, char* argv[])
{
    ...
    g_test_add_func ("/lexer/Bnum", test_lexer_numbers);
    g_test_add_func ("/lexer/Bstr", test_lexer_strings);
    return g_test_run();
}
